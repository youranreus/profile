...
export default function App() {
  ...
  useEffect(() => {
    ...
    getProfile().then((res) => {
      const data = res.data
      data.userInfo.desc = data.userInfo.desc.replace(
        /\${getGrade\((.*?)\)}/g,
        (v, p1) => {
          return getGrade(parseInt(p1))
        }
  ...
  function getGrade(enter) {
    return [0, 'fresh man', 'sophomore', 'junior', 'senior'][
      new Date().getFullYear() - enter
    ]
  }
    ...
    <>
      <div id="main">
        ...
        <Row>
          <Col xs={0} sm={8} md={8} lg={8} xl={8} xxl={8}>
            {!loading && <FloatInfo profile={UserData.userInfo} />}
          </Col>
          <Col xs={24} sm={16} md={16} lg={16} xl={16} xxl={16}>
            <div className={'content'}>
              ...
              <div className="readme">
                <p
                  dangerouslySetInnerHTML={{ __html: UserData.userInfo?.desc }}
                />
              </div>
              ...
              <div className="experience">
                {UserData.experiences &&
                  UserData.experiences.map((item) => (
                    <Experience data={item} key={item.title} />
                  ))}
              </div>
              ...
              <div className="contact">
                ...
                <p>你可以通过以下方式找到我</p>
                <p>
                  <IconMail size={'inherit'} /> youranreus@qq.com |{' '}
                  <IconGithubLogo size={'inherit'} /> @youranreus
                </p>
              </div>
            </div>
          </Col>
      ...
      <footer>
        ...
        <span className="beian">
          <Text link={{ href: 'https://beian.miit.gov.cn/', target: '_blank' }}>
            粤ICP备19008557号-2
          </Text>
        </span>
      </footer>
